...
class CheckpointIO:
    ...
    def load(self, pt_path, verbose=True):
        ...
        data = torch.load(pt_path, map_location=torch.device('cuda'), weights_only=False)
        for name, obj in self._state_objs.items():
            if obj is None:
                pass
            elif isinstance(obj, dict):
    ...
    def load_state_dict(self, pt_path, verbose=True):
        if verbose:
            print0(f'Loading {pt_path} ... ', end='', flush=True)
        ...
        for name, obj in self._state_objs.items():
            if obj is None:
                ...
                try:
                    out = obj.load_state_dict(data[name], strict=False)  
                    print0(f'Warning: Number of unloaded keys: {len(out.missing_keys)}')
                except:
                    # TODO: Lad state_dict for optimizer?
                    pass
            else:
                pass
        if verbose:
    ...
    def load_pretrained(self, run_dir, pattern=r'training-state-(\d+).pt', verbose=True):
        fnames = [entry.name for entry in os.scandir(run_dir) if entry.is_file() and re.fullmatch(pattern, entry.name)]
        if len(fnames) == 0:
            return None
        pt_path = os.path.join(run_dir, max(fnames, key=lambda x: float(re.fullmatch(pattern, x).group(1))))
        self.load_state_dict(pt_path, verbose=verbose)
        return pt_path
