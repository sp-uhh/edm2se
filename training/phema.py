# Code taken from https://github.com/NVlabs/edm2
# 
...
class PowerFunctionEMA:
    ...
    @torch.no_grad()
    def update(self, cur_nwav, batch_size):
        for std, ema in zip(self.stds, self.emas):
            beta = power_function_beta(std=std, t_next=cur_nwav, t_delta=batch_size)
            for p_net, p_ema in zip(self.net.parameters(), ema.parameters()):
    ...
    def load_state_dict(self, state, strict=True):
        ...
        for ema, s_ema in zip(self.emas, state['emas']):
            out = ema.load_state_dict(s_ema, strict=strict)
...
class TraditionalEMA:
    ...
    def update(self, cur_nwav, batch_size):
        ...
        if self.rampup_ratio is not None:
            halflife_Mimg = min(halflife_Mimg, cur_nwav / 1e6 * self.rampup_ratio)
